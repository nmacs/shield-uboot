...
static iomux_v3_cfg_t wfx200_pads[] = {
	MX6_PAD_SD1_DATA2__GPIO2_IO20 | MUX_PAD_CTRL(GPIO_OUT_PAD_CTRL | PAD_CTL_PUS_100K_DOWN),
	MX6_PAD_CSI_VSYNC__GPIO4_IO19 | MUX_PAD_CTRL(GPIO_OUT_PAD_CTRL | PAD_CTL_PUS_100K_DOWN)
};

static void wxf200_setup_sdio(void)
{
	#define WF200_IF_GPIO	IMX_GPIO_NR(2, 20)
	#define WF200_NRST_GPIO	IMX_GPIO_NR(4, 19)
	
	puts("setup sdio\n");
	
	imx_iomux_v3_setup_multiple_pads(wfx200_pads, ARRAY_SIZE(wfx200_pads));
	
	gpio_direction_output(WF200_IF_GPIO,   1);
	gpio_direction_output(WF200_NRST_GPIO, 0);
	
	//udelay(5000);
	
	//gpio_direction_output(WF200_NRST_GPIO, 1);
}
